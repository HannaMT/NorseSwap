...
import { Upload, X, Loader2, ArrowRight, Plus } from 'lucide-react'
  ...
  const onSubmit = async (data) => {
    ...
    if (type === 'SALE') {
      if (data.sale_price === undefined || data.sale_price === null || data.sale_price === '') {
        return toast.error('Sale price is required')
      }
      if (!data.condition) return toast.error('Condition is required')
    }

    if (type === 'RENTAL') {
      if (data.rental_price === undefined || data.rental_price === null || data.rental_price === '') {
        return toast.error('Rental price is required')
      }
      if (!data.rental_period) return toast.error('Rental period is required')
    }
    ...
    try {
      ...
      formData.append('allow_other_campuses', String(!!data.allow_other_campuses))

      appendIfPresent(formData, 'location', data.location?.trim())
      appendIfPresent(formData, 'tags', data.tags?.trim())

      if (type === 'RENTAL') {
        appendIfPresent(formData, 'rental_price_per_period', data.rental_price)
        appendIfPresent(formData, 'rental_price_period', data.rental_period)
        formData.append('rental_deposit', String(data.deposit || 0))
        formData.append('rental_min_days', String(data.min_days || 1))
      } else if (type === 'SALE') {
        appendIfPresent(formData, 'sale_price', data.sale_price)
        appendIfPresent(formData, 'sale_condition', data.condition)
        formData.append('sale_is_negotiable', String(!!data.negotiable))
        formData.append('sale_quantity', String(data.quantity || 1))
      } else if (type === 'SERVICE') {
        appendIfPresent(formData, 'service_category', data.category)
        appendIfPresent(formData, 'service_price_per_hour', data.hourly_rate)
        appendIfPresent(formData, 'service_skill_level', data.skill_level?.trim())
      }

      photos.forEach(({ file }) => formData.append('images', file))

      for (const [key, value] of formData.entries()) {
        console.log('CreateListing payload:', key, value, typeof value)
      }
          ...
          <div className="animate-slide-up space-y-5">
            ...
            <div>
              ...
              <input
                ...
                placeholder={
                  type === 'RENTAL'
                    ? 'e.g. Trek Mountain Bike — Great condition'
                    : type === 'SERVICE'
                      ? 'e.g. Calculus Tutoring — PhD Student'
            ...
            {type === 'RENTAL' && (
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="number" min="1" className="input" placeholder="1" {...register('min_days')} />
                </div>
              </div>
            )}
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    {...register('negotiable')}
                  />
                  <label htmlFor="negotiable" className="font-body text-sm text-brand-ink/70">
                    Price is negotiable
                  </label>
                </div>
                <div>
            ...
            {type === 'SERVICE' && (
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    className="input"
                    placeholder="e.g. PhD Student, 5 years exp."
            ...
            <div>
              ...
              <input className="input" placeholder="e.g. East Campus, Building 32" {...register('location')} />
            </div>

            <div>
              <label className="label">
                Tags <span className="text-brand-ink/30 font-normal">(comma separated)</span>
              </label>
              <input className="input" placeholder="algorithms, cs, textbook" {...register('tags')} />
            </div>
            ...
            >
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                {isDragActive ? (
                  ...
                  <>
                    <p className="font-display font-semibold text-brand-ink">Drag & drop photos here</p>
                ...
                <p className="text-xs text-brand-ink/30 font-mono">{photos.length}/8 photos added</p>
              </div>
            </div>
            ...
            {photos.length > 0 && (
              <div className="grid grid-cols-4 gap-3 mt-4">
                ...
                {photos.length < 8 && (
                  <div
                    {...getRootProps()}
                    className="aspect-square rounded-xl border-2 border-dashed border-black/10 flex items-center justify-center cursor-pointer hover:border-brand-orange/50 transition-colors"
                  >
                    <input {...getInputProps()} />
